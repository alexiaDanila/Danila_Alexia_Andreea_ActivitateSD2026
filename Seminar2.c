...
void copiazaAnumiteElemente(struct Masina* vector, char nrElemente, float prag, struct Masina** vectorNou, int* dimensiune)
{
    ...
    *vectorNou = malloc(sizeof(struct Masina) * *dimensiune);
    int iteratorVectorNou = 0;
    for (int i = 0; i < nrElemente; i++)
    {
        if (vector[i].pret < prag)
        {
            (*vectorNou)[iteratorVectorNou] = vector[i];
            (*vectorNou)[iteratorVectorNou].model = malloc(sizeof(char) * strlen(vector[i].model) + 1);
            strcpy((*vectorNou)[iteratorVectorNou].model, vector[i].model);
            iteratorVectorNou++;
        }
    }
}
